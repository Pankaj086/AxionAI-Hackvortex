...
export const submitAssessmentResult = async (req, res) => {
  try {
    ...
    let existingResult = await AssessmentResult.findOne({
      user: userId,
    ...
    if (existingResult) {
      // Update existing result if found
      existingResult.score = score;
      existingResult.maxScore = maxScore;
      existingResult.token = score;
      existingResult.percentage = percentage;
      existingResult.timeTaken = timeTaken || existingResult.timeTaken;
      existingResult.answers = questionAnswers;
      
      savedResult = await existingResult.save();
      console.log("Updated existing assessment result");
    } else {
      // Create new result document if not found
      ...
      savedResult = await assessmentResult.save();
      console.log("Created new assessment result");
